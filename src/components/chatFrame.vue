<template>
  <div class="kuang">
    ...
    <div class="actionBar">
      <img
        class="emoji"
        :src="require('assets/icon/emoticon.svg')"
        alt=""
        @click="emojiclick"
      />
      <emoji
        :emojiVisible="emojiVisible"
        @emojiVisibleClose="emojiVisibleClose"
      ></emoji>
      <label for="openFile">
...
export default {
  ...
  computed: {
    ...mapGetters("myClient", ["getCleanMessage"]),
  },
  watch: {
    liWidth(newVal) {
      console.log(newVal);
  ...
  methods: {
    ...
    show(value) {
      ...
      if (value.nodeType === 3) {
        ...
        // } else {
        value.text = `<span style="${spanSty.join().replaceAll(",", "")}">${
          value.text
        }</span>`;
        // }
        ...
        if (value.failSend) {
          value.text = `<img
                ...
                alt=""
                style="${failSend.join().replaceAll(",", "")}"
              /><img src=${value.url} width="${
            value.style.width > 180 ? 180 : value.style.width
          }px;"></img>`;
        } else {
          value.text = `<img src=${value.url} width="${
            value.style.width > 180 ? 180 : value.style.width
          }px;"></img>`;
        }
      }
